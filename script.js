const pokemons = [
    ...
    ekranga_chiqarish()
    function ekranga_chiqarish(a=pokemons){
        a.map(function(pokemon){
        let card = document.createElement("div")
        card.classList.add("card")
        card.innerHTML = `  <div class="card-header">
        <span class="id">${pokemon.num}</span>
                        </div>
                        <h3>${pokemon.name}</h3>
                        ...
                        <p class="weakness">${pokemon.weaknesses}</p>
    ...
    btn.addEventListener("click", ()=>{
        cards.innerHTML = ''
        let yangi_array = pokemons.filter(pokemon => pokemon.name.toLowerCase().includes(input.value.toLowerCase()))
        ekranga_chiqarish(yangi_array)
    })

    tugma.addEventListener("click", function(){
        cards.innerHTML = ''
        let yangi_arr = pokemons.filter(pokemon => pokemon.name.toLowerCase().includes(input.value.toLowerCase()))
        if(select.value == "za"){
            yangi_arr.sort((a,z) => z.name.localeCompare(a.name))
          }else if(select.value == "az"){
            yangi_arr.sort((a,z) => a.name.localeCompare(z.name))
          }
        card_render(yangi_arr)
    })

    select.addEventListener("change", function(e) {
        cards.innerHTML = "";
        if (e.target.value == "za") {
            pokemons.sort((a, z) => z.name.localeCompare(a.name));
        } else if (e.target.value == "az") {
            pokemons.sort((a, z) => a.name.localeCompare(z.name));
        }
        ekranga_chiqarish(pokemons);
    });
